  ...
  while( step < int( steps ) )
  {
    ...
    if( z < p.z )
    {
      ...
      {  
        ...
        //NS = -NS;
        vec3 dp = ppos - vec3( p.xy, z );
        dp.x;// /= width; // x in range -1, 1
        dp.y;// /= height; // y in range -1, 1
        float sd = dot( dp, dp );
        float kn = sqrt( abs( dot( NS, N ) ) );
        occl +=  /*( kn / steps ) * */ kn * ( ppos.z - z ) / ( 1. + A * sd ); 
        p.z = z;
        prev = dz / m;
      }      
    }
...
void main()
{
  ...
  if( bool( ssao ) )
  {	
	float r = max( 1.0, projectAtPos( position, dhwidth * radius ) );
	int hw = int( max( 1.0, numSamples / 8.0 ) ); 	
    // project
	vec3 ppos = screenSpace( position );
